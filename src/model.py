...
class Diffusion(nn.Module):
    ...
    def forward(self, inputs):
        ...
        b,c,h,w = x.size()
        # 4 给每个样本随机生成时间步
        t = torch.randint(low=1, high=self.noise_steps + 1, size=(b,)).to(self.device)  
        index = t - 1
...
class DDPM(nn.Module):
    def __init__(
        ...
        self.predict_model = Unet(width = self.width,height = self.height, in_c = self.channel, out_c = self.channel, time_dim = time_dim).to(self.device)

        # register_buffer allows accessing dictionary produced by ddpm_schedules
        # e.g. can access self.sqrtab later
        for k, v in ddpm_schedules(self.beta_start, self.beta_end, self.noise_steps).items():
            self.register_buffer(k, v)

    def sample(self, guide_w = 0.0):
        x_i = torch.randn(1, self.channel, self.height, self.width).to(self.device).repeat(2,1,1,1)
        print()
        for i in range(self.noise_steps, 0, -1):
            print(f'sampling timestep {i}',end='\r')
            
            t_is = torch.tensor([i]).to(self.device).repeat(2)
            z = torch.randn(1, self.channel, self.height, self.width).to(self.device) if i > 1 else 0
